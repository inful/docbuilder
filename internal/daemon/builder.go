...
func (sb *SiteBuilder) Build(ctx context.Context, job *BuildJob) (*hugo.BuildReport, error) {
    if job == nil { return nil, fmt.Errorf("nil job passed to builder") }
    rawCfg, ok := job.Metadata["v2_config"].(*cfg.Config)
    if !ok || rawCfg == nil { return nil, fmt.Errorf("missing v2 configuration in job metadata") }
    ...
    if !ok {
        if ra, ok2 := job.Metadata["repositories"].([]interface{}); ok2 {
            ...
            for _, v := range ra { if r, ok3 := v.(cfg.Repository); ok3 { casted = append(casted, r) } }
            reposAny = casted
        }
    }
    ...
    if err := os.MkdirAll(workspaceDir, 0755); err != nil { return nil, fmt.Errorf("create workspace: %w", err) }
