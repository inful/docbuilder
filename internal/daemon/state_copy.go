...
func CopyRepoState(in *RepoState) *RepoState {
    ...
    if in.Metadata != nil { out.Metadata = copyStringInterfaceMap(in.Metadata) }
    return &out
}
...
func CopyDaemonState(in *DaemonState) *DaemonState {
    ...
    if in.Repositories != nil {
        ...
        for k, v := range in.Repositories { out.Repositories[k] = CopyRepoState(v) }
    }
    if in.Builds != nil {
        out.Builds = make(map[string]*BuildState, len(in.Builds))
        for k, v := range in.Builds { out.Builds[k] = CopyBuildState(v) }
    }
    if in.Schedules != nil {
        out.Schedules = make(map[string]*Schedule, len(in.Schedules))
        for k, v := range in.Schedules { out.Schedules[k] = CopySchedule(v) }
    }
    if in.Statistics != nil { out.Statistics = CopyDaemonStats(in.Statistics) }
    if in.Configuration != nil { out.Configuration = copyStringInterfaceMap(in.Configuration) }
    return &out
}
