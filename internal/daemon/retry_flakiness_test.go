...
func TestRetryFlakinessSmoke(t *testing.T) {
    ...
    for i := 0; i < iterations; i++ {
         t.Run("transient_then_success_iter_"+strconv.Itoa(i), func(t *testing.T) {
            ...
            mb := &mockBuilder{seq: []struct { rep *hugo.BuildReport; err error }{{tr, terr}, {&hugo.BuildReport{}, nil}}}
            ...
            for {
                ...
                if ok && snap.CompletedAt != nil { break }
                if ctx.Err() != nil { t.Fatalf("timeout waiting (transient success) iter %d", i) }
            }
            if got := fr.getRetry(string(hugo.StageCloneRepos)); got != 1 { t.Fatalf("expected 1 retry got %d", got) }
        })
    }
    ...
    for i := 0; i < iterations; i++ {
        t.Run("fatal_no_retry_iter_"+strconv.Itoa(i), func(t *testing.T) {
            ...
            for {
                ...
                snap, ok := bq.JobSnapshot(job.ID)
                if ok && snap.CompletedAt != nil { break }
                if ctx.Err() != nil { t.Fatalf("timeout waiting (fatal no retry) iter %d", i) }
            }
            if got := fr.getRetry(string(hugo.StageCloneRepos)); got != 0 { t.Fatalf("expected 0 retries got %d", got) }
        })
    }
}
