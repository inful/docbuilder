...
func NewRetryPolicy(mode string, initial, max time.Duration, maxRetries int) RetryPolicy {
    ...
    if maxRetries >= 0 { p.MaxRetries = maxRetries }
    if initial > 0 { p.Initial = initial }
    if max > 0 { p.Max = max }
    switch mode {
    ...
    if p.Initial > p.Max { p.Initial = p.Max }
    return p
}
...
func (p RetryPolicy) Validate() error {
    if p.Initial <= 0 { return fmt.Errorf("initial must be >0") }
    if p.Max <= 0 { return fmt.Errorf("max must be >0") }
    if p.MaxRetries < 0 { return fmt.Errorf("max retries cannot be negative") }
    return nil
}
