...
func TestCopyDaemonStateIsolation(t *testing.T) {
    ...
    if cp == orig { t.Fatalf("expected distinct pointer") }
    // mutate copy
    ...
    if orig.Repositories["r1"].Name != "repo1" { t.Fatalf("repo mutation leaked") }
    if orig.Builds["b1"].Status != BuildStatusCompleted { t.Fatalf("build mutation leaked") }
    if orig.Schedules["s1"].Name != "sched" { t.Fatalf("schedule mutation leaked") }
    if orig.Statistics.TotalBuilds != 5 { t.Fatalf("stats mutation leaked") }
    if orig.Configuration["k"].(string) != "v" { t.Fatalf("config mutation leaked") }
}
