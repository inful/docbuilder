...
func TestReportPersistence_Success(t *testing.T) {
    ...
    if err := gen.GenerateSite(files); err != nil { t.Fatalf("build failed: %v", err) }
    jsonPath := filepath.Join(out, "build-report.json")
    if _, err := os.Stat(jsonPath); err != nil { t.Fatalf("expected report json: %v", err) }
    b, _ := os.ReadFile(jsonPath)
    var parsed map[string]any
    if err := json.Unmarshal(b, &parsed); err != nil { t.Fatalf("unmarshal: %v", err) }
    if parsed["outcome"] != "success" { t.Fatalf("expected outcome=success got %v", parsed["outcome"]) }
    if parsed["rendered_pages"].(float64) < 1 { t.Fatalf("expected rendered_pages >=1, got %v", parsed["rendered_pages"]) }
}
...
func TestReportPersistence_FailureDoesNotOverwrite(t *testing.T) {
    ...
    if err := gen.GenerateSite(baseFiles); err != nil { t.Fatalf("initial build failed: %v", err) }
    info, err := os.Stat(filepath.Join(out, "build-report.json"))
    if err != nil { t.Fatalf("missing initial report: %v", err) }
    ...
    if err != nil { t.Fatalf("report disappeared after failed build: %v", err) }
    if info2.ModTime().After(initialMod) { t.Fatalf("report was overwritten on failed build") }
}
