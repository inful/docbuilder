...
func TestGenerateSiteWithReportContextSetsDocFilesHash(t *testing.T) {
    ...
    if err != nil { t.Fatalf("GenerateSiteWithReportContext failed: %v", err) }
    if report == nil { t.Fatalf("expected report") }
    if report.DocFilesHash == "" { t.Fatalf("expected non-empty DocFilesHash") }
    ...
    if !bytes.Contains(data, []byte(report.DocFilesHash)) { t.Fatalf("persisted report missing hash %s", report.DocFilesHash) }
}
