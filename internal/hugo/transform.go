...
type Page struct {
	...
	FrontMatter         map[string]any // (legacy) current working FM (will be replaced by merged output)
...
func (p *Page) applyPatches() {
	...
	for k, v := range p.OriginalFrontMatter { base[k] = v }
	// ensure legacy FrontMatter (built earlier) becomes first patch if present and distinct
	// (backward compatibility while migrating to patches fully)
	if p.FrontMatter != nil && len(p.Patches) == 0 {
		p.Patches = append(p.Patches, FrontMatterPatch{Source: "legacy_front_matter", Mode: MergeDeep, Priority: 10, Data: p.FrontMatter})
	}
	sort.SliceStable(p.Patches, func(i,j int) bool { return p.Patches[i].Priority < p.Patches[j].Priority })
	for _, patch := range p.Patches {
		if patch.Data == nil { continue }
		for k, v := range patch.Data {
			...
			if patch.Mode == MergeDeep {
				if na, okn := toAnySlice(v); okn {
					var oa []any
					if existed { if prevArr, okpa := origVal.([]any); okpa { oa = prevArr } }
					base[k] = mergeArrays(oa, na, patch, p, k)
					if existed { p.Conflicts = append(p.Conflicts, FrontMatterConflict{Key:k, Original:origVal, Attempt:v, Source:patch.Source, Action:"overwritten"}) }
					continue
				}
			}
			base[k] = v
...
func deepMergeMaps(dst map[string]any, src map[string]any, patch FrontMatterPatch, page *Page) map[string]any {
	...
	for k, v := range dst { out[k] = v }
	for k, v := range src {
		if existing, ok := out[k]; ok {
			// recurse maps
			if em, okm := existing.(map[string]any); okm {
				if nm, okn := v.(map[string]any); okn {
					out[k] = deepMergeMaps(em, nm, patch, page); continue
				}
			}
			// arrays
			if ea, okl := toAnySlice(existing); okl {
				if na, okn := toAnySlice(v); okn {
					out[k] = mergeArrays(ea, na, patch, page, k); continue
				}
			}
			// scalar or type mismatch: replace
			if notDeepEqual(existing, v) { page.Conflicts = append(page.Conflicts, FrontMatterConflict{Key:k, Original:existing, Attempt:v, Source:patch.Source, Action:"overwritten"}) }
			out[k] = v
			continue
		}
		out[k] = v
...
func toAnySlice(v any) ([]any, bool) {
	...
	case []any:
		return s, true
	case []string:
		res := make([]any, len(s)); for i, e := range s { res[i] = e }; return res, true
	case []int:
		res := make([]any, len(s)); for i, e := range s { res[i] = e }; return res, true
	default:
		return nil, false
...
func mergeArrays(oldA, newA []any, patch FrontMatterPatch, page *Page, key string) []any {
	...
	if strategy == ArrayReplace { return cloneSlice(newA) }
	if strategy == ArrayAppend { return append(cloneSlice(oldA), newA...) }
	...
	add := func(e any) {
		...
		if keyStr == "" { // unhashable or empty repr—accept duplicates conservatively
			res = append(res, e); return
		}
		if _, ok := seen[keyStr]; ok { return }
		seen[keyStr] = struct{}{}
		res = append(res, e)
	}
	for _, e := range oldA { add(e) }
	for _, e := range newA { add(e) }
	return res
}
...
func stringifyArrayElem(v any) string {
	...
	case int, int8, int16, int32, int64: return fmt.Sprintf("%d", s)
	case uint, uint8, uint16, uint32, uint64: return fmt.Sprintf("%d", s)
	case float32, float64: return fmt.Sprintf("%g", s)
	case bool: if s { return "true" } ; return "false"
	default:
		return "" // unhashable or complex type
	}
}

func cloneSlice(in []any) []any { if in == nil { return nil }; out := make([]any, len(in)); copy(out, in); return out }
	...
	if strings.HasPrefix(body, "---\n") {
		...
		if idx := strings.Index(search, "\n---\n"); idx >= 0 {
			...
			if err := yaml.Unmarshal([]byte(fmContent), &fm); err != nil {
				...
				p.FrontMatter = fm // legacy compatibility path
...
func (m *MergeFrontMatterTransformer) Transform(p *Page) error { p.applyPatches(); p.FrontMatter = p.MergedFrontMatter; return nil }
...
func (e *EditLinkInjector) Transform(p *Page) error {
	if p.OriginalFrontMatter != nil { if _, ok := p.OriginalFrontMatter["editURL"]; ok { return nil } }
	for _, patch := range p.Patches { if patch.Data != nil { if _, ok := patch.Data["editURL"]; ok { return nil } } }
	gen := e.ConfigProvider(); if gen == nil { return nil }
	val := ""
	if gen.editLinkResolver != nil { val = gen.editLinkResolver.Resolve(p.File) } else { val = generatePerPageEditURL(gen.config, p.File) }
	if val == "" { return nil }
