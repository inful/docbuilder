...
func TestGoldenThemeCapabilities(t *testing.T) {
    type row struct { Theme config.Theme `json:"theme"`; EditLinks bool `json:"edit_links"`; SearchJSON bool `json:"search_json"` }
    var rows []row
    for tt, c := range themeCaps { rows = append(rows, row{Theme: tt, EditLinks: c.WantsPerPageEditLinks, SearchJSON: c.SupportsSearchJSON}) }
