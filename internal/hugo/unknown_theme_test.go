...
func TestUnknownThemeFallback(t *testing.T) {
    ...
    if err := os.MkdirAll(g.stageDir, 0o755); err != nil { t.Fatalf("mkdir staging: %v", err) }
    if err := g.generateHugoConfig(); err != nil { t.Fatalf("generate config: %v", err) }
    data, err := os.ReadFile(filepath.Join(g.buildRoot(), "hugo.yaml"))
    if err != nil { t.Fatalf("read hugo.yaml: %v", err) }
    content := string(data)
    if len(content) == 0 { t.Fatalf("empty hugo.yaml for unknown theme") }
    if contains(content, "module:") { t.Fatalf("unexpected module block for unknown theme:\n%s", content) }
}
