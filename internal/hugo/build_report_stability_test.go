...
func TestBuildReportStability(t *testing.T) {
    ...
    if ser.DocFilesHash == "" { ser.DocFilesHash = "" }
    if ser.DeltaDecision == "" { ser.DeltaDecision = "" }
    if ser.DeltaChangedRepos == nil { ser.DeltaChangedRepos = nil }
    if ser.DeltaRepoReasons == nil { ser.DeltaRepoReasons = nil }
    ...
    if err := json.Unmarshal(golden, &gMap); err != nil { t.Fatalf("unmarshal golden: %v", err) }
    if err := json.Unmarshal(gotJSON, &cMap); err != nil { t.Fatalf("unmarshal current: %v", err) }
    // Keys we require to stay stable (subset).
    required := []string{"schema_version","repositories","files","outcome","cloned_repositories","rendered_pages","config_hash"}
