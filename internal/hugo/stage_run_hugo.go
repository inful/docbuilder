...
func stageRunHugo(ctx context.Context, bs *BuildState) error {
	...
	if mode == config.RenderModeNever { return nil }
	if !shouldRunHugo(cfg) { return nil }
	...
	if bs.Generator.renderer == nil {
		if err := bs.Generator.runHugoBuild(); err != nil { return newWarnStageError(StageRunHugo, fmt.Errorf("%w: %v", build.ErrHugo, err)) }
