...
func (g *Generator) generateHugoConfig() error {
	...
	if g.config.Hugo.Params != nil { mergeParams(params, g.config.Hugo.Params) }
	...
	if features.EnableMathPassthrough {
		if m, ok := root["markup"].(map[string]any); ok {
			...
			if gm == nil { gm = map[string]any{}; m["goldmark"] = gm }
			ext, _ := gm["extensions"].(map[string]any)
			if ext == nil { ext = map[string]any{}; gm["extensions"] = ext }
	...
	if th := g.activeTheme(); th != nil { th.CustomizeRoot(g, root) }
	...
	if features.UsesModules { if err := g.ensureGoModForModules(); err != nil { slog.Warn("Failed to ensure go.mod for Hugo Modules", "error", err) } }
