...
func repo(url, name, branch string, paths ...string) config.Repository {
    if len(paths) == 0 { paths = []string{"docs"} }
...
func TestEditLinkResolver_BasicForgeTypes(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            if tt.wantEmpty {
                if got != "" { t.Fatalf("expected empty edit link, got %q", got) }
                return
            }
            if got == "" { t.Fatalf("expected non-empty edit link") }
            if got != tt.wantPrefix { t.Fatalf("edit link mismatch\n got: %s\nwant: %s", got, tt.wantPrefix) }
        })
    }
}
