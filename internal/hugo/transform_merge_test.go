...
func TestFrontMatterDeepMergeNested(t *testing.T) {
	...
	if a["b"].(int) != 1 || a["f"].(int) != 4 {
		t.Fatalf("expected preserved and added keys in nested merge: %#v", a)
	}
	inner := a["c"].(map[string]any)
	if inner["d"].(int) != 2 || inner["e"].(int) != 3 {
		t.Fatalf("expected deep nested merge: %#v", inner)
	}
	tags := p.MergedFrontMatter["tags"].([]any)
	if len(tags) != 2 {
		t.Fatalf("expected union of tags, got %v", tags)
	}
}
...
func TestReservedKeyProtection(t *testing.T) {
	...
	p.applyPatches()
	if p.MergedFrontMatter["title"].(string) != "Original" {
		t.Fatalf("reserved key title should be protected")
	}
	if p.MergedFrontMatter["description"].(string) != "Desc" {
		t.Fatalf("reserved key description should be protected")
	}
	if len(p.Conflicts) == 0 {
		t.Fatalf("expected conflicts recorded")
	}
}
...
func TestSetIfMissingAndReplace(t *testing.T) {
	...
	p.applyPatches()
	if p.MergedFrontMatter["slug"].(string) != "override" {
		t.Fatalf("expected replace to override slug")
	}
	if p.MergedFrontMatter["newkey"].(string) != "val" {
		t.Fatalf("expected newkey set")
	}
}
...
func TestBuilderIntegrationConflictRecording(t *testing.T) {
	...
	p.applyPatches()
	if p.MergedFrontMatter["title"].(string) != "Keep" {
		t.Fatalf("title should remain original")
	}
	if p.MergedFrontMatter["other"].(int) != 1 {
		t.Fatalf("other key should be set")
	}
	found := false
	for _, c := range p.Conflicts {
		if c.Key == "title" && c.Action == "kept_original" {
			found = true
		}
	}
	if !found {
		t.Fatalf("expected kept_original conflict for title")
	}
}
