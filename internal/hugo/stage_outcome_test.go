...
func TestClassifyStageResult_WrappedClonePartial(t *testing.T) {
	...
	if out.IssueCode != IssuePartialClone { t.Fatalf("expected partial clone, got %s", out.IssueCode) }
	if out.Result != StageResultWarning || out.Abort { t.Fatalf("expected warning non-abort: %+v", out) }
}
...
func TestClassifyStageResult_UnknownFatal(t *testing.T) {
	...
	if out.IssueCode != IssueGenericStageError { t.Fatalf("expected generic code, got %s", out.IssueCode) }
	if out.Result != StageResultFatal || !out.Abort { t.Fatalf("expected fatal abort %+v", out) }
}
