...
type dummyGenerator struct{ cfg *config.Config }
...
func TestEditLinkInjectorV2_HextraAdds(t *testing.T) {
    ...
    SetGeneratorProvider(func() any { return dummyGenerator{cfg: cfg} })
    doc := testDocFile()
    shim := &PageShim{Doc: doc}
    // Run builder then edit link injector
    if err := (FrontMatterBuilderV2{}).Transform(shim); err != nil { t.Fatalf("builder v2 error: %v", err) }
    if err := (EditLinkInjectorV2{}).Transform(shim); err != nil { t.Fatalf("edit link v2 error: %v", err) }
    ...
    if !found { t.Fatalf("expected edit_link_v2 patch to be added; patches=%+v", shim.Patches) }
}
