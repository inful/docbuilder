...
func Register(t Transformer) { if t != nil { if _, ok := reg[t.Name()]; !ok { reg[t.Name()] = t } } }
...
func List() []Transformer {
  ...
  for _, t := range reg { items = append(items, t) }
  sort.SliceStable(items, func(i,j int) bool { if items[i].Priority() == items[j].Priority() { return items[i].Name() < items[j].Name() }; return items[i].Priority() < items[j].Priority() })
  return items
}
...
func BuildPipeline(include map[string]struct{}) ([]Transformer, error) {
  all := List()
  if include == nil || len(include) == 0 { return all, nil }
  var out []Transformer
  for _, t := range all { if _, ok := include[t.Name()]; ok { out = append(out, t) } }
