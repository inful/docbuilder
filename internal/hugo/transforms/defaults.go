...
func (t FrontMatterParser) Transform(p PageAdapter) error {
  pg, ok := p.(*PageShim); if !ok { return nil }
  body := pg.Content
...
func (t FrontMatterBuilder) Transform(p PageAdapter) error {
  ...
  if shim, ok := p.(*PageShim); ok {
    if shim.BuildFrontMatter != nil { shim.BuildFrontMatter(time.Now()) }
  }
  return nil
}
...
func (t EditLinkInjector) Transform(p PageAdapter) error {
  if shim, ok := p.(*PageShim); ok { if shim.InjectEditLink != nil { shim.InjectEditLink() } }
  return nil
}
...
func (t MergeFrontMatter) Transform(p PageAdapter) error {
  if shim, ok := p.(*PageShim); ok { if shim.ApplyPatches != nil { shim.ApplyPatches() } }
  return nil
}
...
func (t RelativeLinkRewriter) Transform(p PageAdapter) error {
  if shim, ok := p.(*PageShim); ok { if shim.RewriteLinks != nil { shim.Content = shim.RewriteLinks(shim.Content) } }
  return nil
}
...
func (t Serializer) Transform(p PageAdapter) error {
  if shim, ok := p.(*PageShim); ok { if shim.Serialize != nil { return shim.Serialize() } }
  return nil
}
