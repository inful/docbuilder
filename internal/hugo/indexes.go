...
func (g *Generator) generateIndexPages(docFiles []docs.DocFile) error {
    if err := g.generateMainIndex(docFiles); err != nil { return err }
    if err := g.generateRepositoryIndexes(docFiles); err != nil { return err }
    if err := g.generateSectionIndexes(docFiles); err != nil { return err }
    return nil
}
...
func (g *Generator) generateMainIndex(docFiles []docs.DocFile) error {
    ...
    if g.config.Hugo.Theme == "hextra" { frontMatter["cascade"] = map[string]interface{}{"type": "docs"} }
...
func (g *Generator) generateRepositoryIndexes(docFiles []docs.DocFile) error {
    ...
    for repoName, files := range repoGroups {
        ...
        for _, file := range files { section := file.Section; if section == "" { section = "root" }; sectionGroups[section] = append(sectionGroups[section], file) }
...
func (g *Generator) generateSectionIndexes(docFiles []docs.DocFile) error {
    ...
    for _, file := range docFiles {
        ...
        if sectionGroups[file.Repository] == nil { sectionGroups[file.Repository] = make(map[string][]docs.DocFile) }
    ...
    for repoName, sections := range sectionGroups {
        for sectionName, files := range sections {
            ...
            for _, file := range files { title := titleCase(strings.ReplaceAll(file.Name, "-", " ")); content += fmt.Sprintf("- [%s](./%s/)\n", title, file.Name) }
            if err := os.WriteFile(indexPath, []byte(content), 0644); err != nil { return fmt.Errorf("failed to write section index: %w", err) }
