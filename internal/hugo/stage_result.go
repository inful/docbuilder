...
func (r *BuildReport) recordStageResult(stage StageName, res StageResult, recorder metrics.Recorder) {
    ...
    case StageResultSuccess:
        sc.Success++
        if recorder != nil { recorder.IncStageResult(string(stage), metrics.ResultSuccess) }
    case StageResultWarning:
        sc.Warning++
        if recorder != nil { recorder.IncStageResult(string(stage), metrics.ResultWarning) }
    case StageResultFatal:
        sc.Fatal++
        if recorder != nil { recorder.IncStageResult(string(stage), metrics.ResultFatal) }
    case StageResultCanceled:
        sc.Canceled++
        if recorder != nil { recorder.IncStageResult(string(stage), metrics.ResultCanceled) }
    }
