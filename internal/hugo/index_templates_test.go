...
func TestIndexTemplates_FallbackEmbedded(t *testing.T) {
    ...
    if err := os.MkdirAll(filepath.Join(g.outputDir, "content"), 0o755); err != nil { t.Fatalf("mkdir content: %v", err) }
    if err := g.generateIndexPages(files); err != nil { t.Fatalf("generate indexes: %v", err) }
    mainIdx := filepath.Join(g.outputDir, "content", "_index.md")
    b, err := os.ReadFile(mainIdx)
    if err != nil { t.Fatalf("read main index: %v", err) }
    if !strings.Contains(string(b), "# Test Site") { t.Fatalf("expected embedded template output, got:\n%s", string(b)) }
}
...
func TestIndexTemplates_UserOverridePrecedence(t *testing.T) {
    ...
    if err := os.MkdirAll(overrideDir, 0o755); err != nil { t.Fatalf("mkdir override: %v", err) }
    if err := os.MkdirAll(filepath.Join(g.outputDir, "content"), 0o755); err != nil { t.Fatalf("mkdir content: %v", err) }
    content := "CUSTOM MAIN TEMPLATE\n"
    if err := os.WriteFile(filepath.Join(overrideDir, "main.md.tmpl"), []byte(content), 0o644); err != nil { t.Fatalf("write override: %v", err) }
    files := []docs.DocFile{{Repository: "repo1", Name: "doc1", Path: "doc1.md"}}
    if err := g.generateIndexPages(files); err != nil { t.Fatalf("generate indexes: %v", err) }
    mainIdx := filepath.Join(g.outputDir, "content", "_index.md")
    b, err := os.ReadFile(mainIdx)
    if err != nil { t.Fatalf("read main index: %v", err) }
    if !strings.Contains(string(b), "CUSTOM MAIN TEMPLATE") { t.Fatalf("expected override template content, got:\n%s", string(b)) }
}
