...
func (m markerTransform) Transform(p tr.PageAdapter) error {
  if shim, ok := p.(*tr.PageShim); ok { shim.Content = shim.Content + "\nMARKER" }
  return nil
}
...
func TestTransformFiltering_EnableDisable(t *testing.T) {
  ...
  if err := g.copyContentFiles(context.Background(), []docs.DocFile{df}); err != nil { t.Fatalf("copyContentFiles: %v", err) }
  // Read generated file
  outPath := g.buildRoot() + "/" + df.GetHugoPath()
  data, err := os.ReadFile(outPath)
  if err != nil { t.Fatalf("read output: %v", err) }
  if strings.Contains(string(data), "MARKER") { t.Fatalf("marker transform should have been disabled\n%s", string(data)) }
  ...
  if err := g2.copyContentFiles(context.Background(), []docs.DocFile{df}); err != nil { t.Fatalf("copyContentFiles(2): %v", err) }
  outPath2 := g2.buildRoot() + "/" + df.GetHugoPath()
  data2, err := os.ReadFile(outPath2)
  if err != nil { t.Fatalf("read output2: %v", err) }
  if !strings.Contains(string(data2), "MARKER") { t.Fatalf("marker transform should have run\n%s", string(data2)) }
