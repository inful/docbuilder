...
func (g *Generator) ensureGoModForModules() error {
    ...
    deriveModuleName := func() string {
        ...
        if g.config.Hugo.BaseURL != "" {
            ...
            host := s
            if idx := strings.IndexByte(s, '/'); idx >= 0 { host = s[:idx] }
            if p := strings.IndexByte(host, ':'); p >= 0 { host = host[:p] }
            if host != "" { moduleName = strings.ReplaceAll(host, ".", "-") }
        }
        return moduleName
    }
        ...
        if readErr == nil {
            ...
            if len(lines) > 0 && strings.HasPrefix(lines[0], "module ") {
                ...
                if strings.Contains(existing, ":") {
                    ...
                    rest := ""
                    if len(lines) > 1 { rest = lines[1] }
                    newContent := fmt.Sprintf("module %s\n", sanitized)
                    if !strings.Contains(rest, "go ") { newContent += "\ngo 1.21\n" } else { newContent += rest }
    ...
    if err := os.WriteFile(goModPath, []byte(content), 0644); err != nil { return fmt.Errorf("failed to write go.mod: %w", err) }
