...
func TestPipeline_Idempotency(t *testing.T) {
    ...
    if err := pipe.Run(p); err != nil { t.Fatalf("first run failed: %v", err) }

    firstOutput := string(p.Raw)
    if strings.Count(firstOutput, "---\n") < 1 { t.Fatalf("expected front matter once, got: %s", firstOutput) }
    if strings.Contains(firstOutput, "doc.md") { t.Fatalf("link rewrite failed: %s", firstOutput) }
    ...
    if err := pipe.Run(p2); err != nil { t.Fatalf("second run failed: %v", err) }
    secondOutput := string(p2.Raw)

    if strings.Count(secondOutput, "---\n") != 1 { t.Fatalf("front matter duplicated after second run: %s", secondOutput) }
}
...
func TestPipeline_Order(t *testing.T) {
    ...
    if err := pipe.Run(p); err != nil { t.Fatalf("pipeline failed: %v", err) }
    out := string(p.Raw)
    if !strings.Contains(out, "custom: val") { t.Fatalf("existing front matter key lost: %s", out) }
    if !strings.Contains(out, "repository: r") { t.Fatalf("builder did not add repository: %s", out) }
}
...
func TestMalformedFrontMatter(t *testing.T) {
    ...
    if err := pipe.Run(p); err != nil { t.Fatalf("pipeline failed: %v", err) }
    out := string(p.Raw)
    if !strings.Contains(out, "title:") { t.Fatalf("expected generated front matter title, got: %s", out) }
}
...
func TestDateConsistency(t *testing.T) {
    ...
    if err := pipe.Run(p); err != nil { t.Fatalf("pipeline failed: %v", err) }
    if !strings.Contains(string(p.Raw), "date:") { t.Fatalf("expected date in serialized output, got %s", string(p.Raw)) }
}
