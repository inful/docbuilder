...
func TestBuildFrontMatter_TitleAndBasicFields(t *testing.T) {
    ...
    if fm["title"] != "Getting Started" { t.Fatalf("expected title 'Getting Started', got %v", fm["title"]) }
    if fm["repository"] != "repo1" { t.Fatalf("repository not set correctly: %v", fm["repository"]) }
    if fm["section"] != "guide" { t.Fatalf("section not set: %v", fm["section"]) }
    if fm["date"] == nil { t.Fatalf("date should be set") }
}
...
func TestBuildFrontMatter_IndexNoTitle(t *testing.T) {
    ...
    if _, exists := fm["title"]; exists { t.Fatalf("index file should not auto-generate title, got %v", fm["title"]) }
}
...
func TestBuildFrontMatter_MetadataPassthrough(t *testing.T) {
    ...
    fm := BuildFrontMatter(FrontMatterInput{File: file, Config: cfg, Now: fixedTime()})
    if fm["product"] != "alpha" { t.Fatalf("metadata not passed through: %v", fm["product"]) }
}
...
func TestBuildFrontMatter_EditURL_GitHub(t *testing.T) {
    ...
    if fm["editURL"] != want { t.Fatalf("expected editURL %s got %v", want, fm["editURL"]) }
}
...
func TestBuildFrontMatter_EditURL_GitLabSSH(t *testing.T) {
    ...
    if fm["editURL"] != want { t.Fatalf("expected %s got %v", want, fm["editURL"]) }
}
...
func TestBuildFrontMatter_EditURL_SiteBaseSuppressesPerPage(t *testing.T) {
    ...
    fm := BuildFrontMatter(FrontMatterInput{File: file, Config: cfg, Now: fixedTime()})
    if _, exists := fm["editURL"]; exists { t.Fatalf("per-page editURL should be suppressed when site base provided, got %v", fm["editURL"]) }
}
...
func TestBuildFrontMatter_ExistingPreserved(t *testing.T) {
    ...
    if fm["title"] != "Custom" { t.Fatalf("existing title should be preserved, got %v", fm["title"]) }
    if fm["editURL"] != "https://override" { t.Fatalf("existing editURL should be preserved, got %v", fm["editURL"]) }
}
