...
func newBuildReport(repos, files int) *BuildReport {
    return &BuildReport{Repositories: repos, Files: files, Start: time.Now(), StageDurations: make(map[string]time.Duration), StageErrorKinds: make(map[string]string)}
}
...
func (r *BuildReport) Summary() string {
    ...
    return fmt.Sprintf("repos=%d files=%d duration=%s errors=%d warnings=%d stages=%d", r.Repositories, r.Files, dur.Truncate(time.Millisecond), len(r.Errors), len(r.Warnings), len(r.StageDurations))
}
