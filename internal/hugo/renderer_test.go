...
// TestNoopRenderer ensures that when a stages.NoopRenderer is injected the RunHugo stage
// marks the site as rendered without invoking the external hugo binary (which may
// not be installed in CI). We approximate this by injecting stages.NoopRenderer and using
...
func TestNoopRenderer(t *testing.T) {
	...
	g := NewGenerator(&config.Config{Hugo: config.HugoConfig{Title: "Test", BaseURL: "/"}}, dir).WithRenderer(&stages.NoopRenderer{})
	...
	if err != nil {
		// Any hugo invocation attempt (if stages.NoopRenderer not used) could fail here if binary missing.
		// Surface error for visibility.
		t.Fatalf("generation failed: %v", err)
	}

	if !report.StaticRendered {
		t.Fatalf("expected report.StaticRendered=true with stages.NoopRenderer, got false")
	}

	// With stages.NoopRenderer no static site is produced; we only assert that the pipeline considered rendering done.
}
