...
func TestGenerateSite_Smoke(t *testing.T) {
    ...
    if _, err := os.Stat(introPath); err != nil { t.Fatalf("intro file missing: %v", err) }
    if _, err := os.Stat(guidePath); err != nil { t.Fatalf("guide file missing: %v", err) }

    b, err := os.ReadFile(introPath)
    if err != nil { t.Fatalf("read intro: %v", err) }
    content := string(b)

    // Front matter delimiter
    if !strings.HasPrefix(content, "---\n") { t.Fatalf("front matter missing at start: %s", content[:30]) }
    if !strings.Contains(content, "repository: repo1") { t.Fatalf("repository not in front matter: %s", content) }
    if !strings.Contains(content, "editURL:") { t.Fatalf("expected editURL in front matter for hextra") }
    if strings.Contains(content, "guide.md") { t.Fatalf("link rewriting failed to strip .md: %s", content) }

    // Ensure date field present (format not strictly validated here for simplicity)
    if !strings.Contains(content, "date:") { t.Fatalf("date missing in front matter") }

    // Sanity: guide file also has front matter
    gb, err := os.ReadFile(guidePath)
    if err != nil { t.Fatalf("read guide: %v", err) }
