...
func TestFrontMatterConflicts(t *testing.T) {
  ...
  if p.MergedFrontMatter["title"] != "Existing Title" { t.Fatalf("title changed unexpectedly: %#v", p.MergedFrontMatter["title"]) }
  if p.MergedFrontMatter["description"] != "Replaced Desc" { t.Fatalf("description not replaced: %#v", p.MergedFrontMatter["description"]) }
  if p.MergedFrontMatter["weight"].(int) != 10 { t.Fatalf("weight should remain original (10), got %#v", p.MergedFrontMatter["weight"]) }
  if p.MergedFrontMatter["summary"] != "Short summary" { t.Fatalf("summary missing: %#v", p.MergedFrontMatter["summary"]) }
  // tags union order stable: [alpha beta]
  tags, _ := p.MergedFrontMatter["tags"].([]any)
  if len(tags) != 2 || tags[0] != "alpha" || tags[1] != "beta" { t.Fatalf("unexpected tags union result: %#v", tags) }
}
