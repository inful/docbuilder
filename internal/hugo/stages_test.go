...
func TestStageRunnerTimings(t *testing.T) {
	...
	if err != nil { t.Fatalf("GenerateSiteWithReport error: %v", err) }
	if len(report.StageDurations) == 0 { t.Fatalf("expected stage durations recorded") }
	var sum time.Duration
	for name, d := range report.StageDurations { if d <= 0 { t.Errorf("stage %s duration zero", name) }; sum += d }
	if report.End.Sub(report.Start) < sum { t.Errorf("total duration %s < sum of stages %s", report.End.Sub(report.Start), sum) }
}
