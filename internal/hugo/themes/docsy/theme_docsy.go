...
func (Theme) Features() hugo.ThemeFeatures { return hugo.ThemeFeatures{
    Name: config.ThemeDocsy, UsesModules: true, ModulePath: "github.com/google/docsy",
    EnableOfflineSearchJSON: true,
}}
func (Theme) ApplyParams(g *hugo.Generator, params map[string]any) {
    ...
    if params["links"] == nil {
        ...
        for _, repo := range g.Config().Repositories {
            if strings.Contains(repo.URL, "github.com") {
                link := map[string]any{"name": fmt.Sprintf("%s Repository", hugo.TitleCase(repo.Name)), "url": repo.URL, "icon": "fab fa-github", "desc": fmt.Sprintf("Development happens here for %s", repo.Name)}
...
func (Theme) CustomizeRoot(g *hugo.Generator, root map[string]any) {}

func init() { hugo.RegisterTheme(Theme{}) }
