...
func addSimpleCommit(t *testing.T, repo *git.Repository, repoPath, name string) plumbing.Hash {
	...
	if err != nil { t.Fatalf("worktree: %v", err) }
	file := filepath.Join(repoPath, name)
	if err := os.WriteFile(file, []byte(name), 0o644); err != nil { t.Fatalf("write: %v", err) }
	if _, err := wt.Add(name); err != nil { t.Fatalf("add: %v", err) }
...
func TestResolveTargetBranchExplicit(t *testing.T) {
	...
	if b != "feature-x" { t.Fatalf("expected feature-x got %s", b) }
}
...
func TestResolveTargetBranchFromHead(t *testing.T) {
	...
	if b != "master" && b != "main" { t.Fatalf("expected master or main got %s", b) }
}
...
func TestUpdateExistingRepoFastForward(t *testing.T) {
	...
	if _, err := git.PlainInit(bare, true); err != nil { t.Fatalf("init bare: %v", err) }
	// seed working with commit A then push
	seedPath := filepath.Join(tmp, "seed")
	seedRepo, err := git.PlainInit(seedPath, false)
	if err != nil { t.Fatalf("init seed: %v", err) }
	if _, err := seedRepo.CreateRemote(&ggitcfg.RemoteConfig{Name: "origin", URLs: []string{bare}}); err != nil { t.Fatalf("remote: %v", err) }
	addSimpleCommit(t, seedRepo, seedPath, "a.txt")
	if err := seedRepo.Push(&git.PushOptions{RemoteName: "origin"}); err != nil { t.Fatalf("push A: %v", err) }

	// clone local to update later
	localPath := filepath.Join(tmp, "local")
	if _, err := git.PlainClone(localPath, false, &git.CloneOptions{URL: bare}); err != nil { t.Fatalf("clone: %v", err) }
	// add commit B to remote (via seed) and push
	addSimpleCommit(t, seedRepo, seedPath, "b.txt")
	if err := seedRepo.Push(&git.PushOptions{RemoteName: "origin"}); err != nil { t.Fatalf("push B: %v", err) }
	...
	if _, err := client.updateExistingRepo(localPath, repoCfg); err != nil { t.Fatalf("updateExistingRepo fast-forward: %v", err) }
	updated, _ := git.PlainOpen(localPath)
	head, _ := updated.Head()
	if head.Hash() != remoteHash { t.Fatalf("expected fast-forward head %s remote %s", head.Hash(), remoteHash) }
}
