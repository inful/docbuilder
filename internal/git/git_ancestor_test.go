...
func addCommit(t *testing.T, repo *git.Repository, repoPath, filename, content, msg string) plumbing.Hash {
    ...
    if err := os.WriteFile(full, []byte(content), 0o644); err != nil { t.Fatalf("write file: %v", err) }
    if _, err := wt.Add(filename); err != nil { t.Fatalf("add: %v", err) }
...
func TestIsAncestorEdgeCases(t *testing.T) {
    ...
    if err != nil || !same { t.Fatalf("expected identical hash ancestor true, got %v err=%v", same, err) }

    // 2. A is ancestor of C
    res, err := isAncestor(repo, a, c)
    if err != nil || !res { t.Fatalf("expected A ancestor of C: res=%v err=%v", res, err) }

    // 3. C is not ancestor of A
    res, err = isAncestor(repo, c, a)
    if err != nil { t.Fatalf("unexpected error reverse direction: %v", err) }
    if res { t.Fatalf("expected C not ancestor of A") }
    ...
    if err != nil { t.Fatalf("unexpected error for missing ancestor hash: %v", err) }
    if res { t.Fatalf("expected false for missing ancestor hash") }
    ...
    if err == nil { t.Fatalf("expected error for nonexistent starting commit b") }
    ...
    if !res || err != nil { t.Fatalf("expected true (early equality) for identical nonexistent hash, got res=%v err=%v", res, err) }
}
