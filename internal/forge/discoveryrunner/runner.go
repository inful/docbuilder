...
// BuildEnqueuer is the port the runner uses to schedule a build after
// discovery finds new repositories. The queue package implements this
// via DiscoveryEnqueuerAdapter; tests can provide their own.
//
// This indirection lets the runner stay out of the build/queue package
// (it never touches queue.BuildJob literals) while still supporting
// end-to-end build triggering. The jobID is generated by the runner
// (via its Config.NewJobID callback) and passed in so callers can
// correlate logs and metrics with the same identifier.
type BuildEnqueuer interface {
	EnqueueDiscoveryBuild(ctx context.Context, jobID string, repos []config.Repository, cfg *config.Config) error
}
...
//
// If set, the runner will call it instead of enqueuing via BuildEnqueuer.
...
func (r *Runner) triggerBuildForDiscoveredRepos(ctx context.Context, result *forge.DiscoveryResult) {
	...
	if err := r.buildQueue.EnqueueDiscoveryBuild(ctx, jobID, converted, r.config); err != nil {
		slog.Error("Failed to enqueue auto-build", logfields.JobID(jobID), logfields.Error(err))
	}
}
