...
func TestGoldenForgeCapabilities(t *testing.T) {
    type row struct { Forge config.ForgeType `json:"forge"`; SupportsEditLinks bool `json:"edit_links"`; SupportsWebhooks bool `json:"webhooks"`; SupportsTopics bool `json:"topics"` }
    var rows []row
    for ft, c := range caps { rows = append(rows, row{Forge: ft, SupportsEditLinks: c.SupportsEditLinks, SupportsWebhooks: c.SupportsWebhooks, SupportsTopics: c.SupportsTopics}) }
