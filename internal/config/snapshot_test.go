...
func baseCfg() *Config { return &Config{Version: "2.0", Hugo: HugoConfig{Theme: "hextra", BaseURL: "https://example", Title: "Docs"}} }
...
func TestSnapshotStableAcrossNormalizationVariants(t *testing.T) {
    ...
    if _, err := NormalizeConfig(a); err != nil { t.Fatalf("normalize a: %v", err) }
    if err := applyDefaults(a); err != nil { t.Fatalf("defaults a: %v", err) }
    snapA := a.Snapshot()
    ...
    if _, err := NormalizeConfig(b); err != nil { t.Fatalf("normalize b: %v", err) }
    if err := applyDefaults(b); err != nil { t.Fatalf("defaults b: %v", err) }
    snapB := b.Snapshot()

    if snapA != snapB { t.Fatalf("expected snapshots equal, got\nA=%s\nB=%s", snapA, snapB) }
}
...
func TestSnapshotDetectsMeaningfulChange(t *testing.T) {
    ...
    if snap1 == snap2 { t.Fatalf("expected snapshot change after render_mode modification") }
}